...
void key_irq_work(struct work_struct *w)
{
	printk("-----------^_^ %s-------------\n",__FUNCTION__);
	key_dev->have_data = 1;
	wake_up_interruptible(&key_dev->wq); //唤醒阻塞的进程
...
irqreturn_t key_irq_thread(int irqno, void * dev_id)
{
	printk("-----------^_^ %s-------------\n",__FUNCTION__);
	key_dev->have_data = 1;
	wake_up_interruptible(&key_dev->wq); //唤醒阻塞的进程
...
int key_pdrv_probe(struct platform_device *pdev)
{
	...
	ret = request_threaded_irq(key_dev->irqno, key_irq_fun, key_irq_thread, IRQF_TRIGGER_FALLING|IRQF_TRIGGER_RISING, "key1", NULL);
	if(ret){
	...
	//初始化tasklet
	// tasklet_init(&key_dev->tasklet, key_irq_tasklet, 120);

	//初始化工作队列
	//INIT_WORK(&key_dev->work, key_irq_work);
